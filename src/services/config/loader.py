...
Unified configuration loading for all ThinkPilot modules.
...
# PROJECT_ROOT points to the actual project root directory (ThinkPilot/)
# Path(__file__) = src/services/config/loader.py
# .parent = src/services/config/
# .parent.parent = src/services/
# .parent.parent.parent = src/
# .parent.parent.parent.parent = ThinkPilot/ (project root)
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent

# Backward-compatible aliases after consolidating module configs into main.yaml.
LEGACY_CONFIG_ALIASES = {
    "solve_config.yaml": "main.yaml",
    "research_config.yaml": "main.yaml",
    "guide_config.yaml": "main.yaml",
}


def _resolve_config_path(config_dir: Path, config_file: str) -> Path:
    """Resolve config filename and fall back for legacy names."""
    direct_path = config_dir / config_file
    if direct_path.exists():
        return direct_path

    alias_name = LEGACY_CONFIG_ALIASES.get(config_file)
    if alias_name:
        alias_path = config_dir / alias_name
...
def _load_yaml_file(file_path: Path) -> dict[str, Any]:
    ...
    stat = file_path.stat()
    return copy.deepcopy(_load_yaml_file_cached(str(file_path), stat.st_mtime_ns, stat.st_size))


@lru_cache(maxsize=32)
def _load_yaml_file_cached(path: str, mtime_ns: int, size: int) -> dict[str, Any]:
    """
    Cached YAML loader keyed by path and file metadata.

    The cache is invalidated automatically when a file changes (mtime/size).
    """
    del mtime_ns, size  # metadata is part of cache key; avoid accidental unused warnings
...
def load_config_with_main(config_file: str, project_root: Path | None = None) -> dict[str, Any]:
    ...
    module_config = {}
    module_config_path = _resolve_config_path(config_dir, config_file)
...
def get_agent_params(module_name: str) -> dict:
    ...
    try:
        ...
        if config_path.exists():
            agents_config = _load_yaml_file(config_path)
