...
class UnifiedConfigManager:
    ...
    def __init__(self):
        ...
        self._storage_cache: Dict[str, tuple[int, int, Dict[str, Any]]] = {}
    ...
    def _load_configs(self, config_type: ConfigType) -> Dict[str, Any]:
        ...
        default_data = {"configs": [], "active_id": "default"}

        with self._storage_lock:
            if not path.exists():
                self._storage_cache.pop(cache_key, None)
            ...
            try:
                ...
                self._storage_cache[cache_key] = (stat.st_mtime_ns, stat.st_size, data)
    ...
    def _save_configs(self, config_type: ConfigType, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            with self._storage_lock:
                fd, tmp_path = tempfile.mkstemp(prefix=f"{cache_key}.", suffix=".tmp", dir=str(path.parent))
                try:
                ...
                stat = path.stat()
                self._storage_cache[cache_key] = (stat.st_mtime_ns, stat.st_size, payload)
            return True
