...
def _empty_subject_defaults() -> Dict[str, Optional[str]]:
    return {subject: None for subject in SUPPORTED_SUBJECTS}
...
class KnowledgeBaseConfigService:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if config is None:
            config = {
                "configs": {},
                "default_kb": None,
                "subject_defaults": _empty_subject_defaults(),
                "global_defaults": {
        ...
        subject_defaults = config.get("subject_defaults")
        normalized_defaults = _empty_subject_defaults()
        if isinstance(subject_defaults, dict):
            for subject, kb_name in subject_defaults.items():
                try:
                    normalized_subject = normalize_subject(subject)
                except ValueError:
                    continue
                normalized_defaults[normalized_subject] = kb_name
        config["subject_defaults"] = normalized_defaults

        if "default_kb" not in config:
            config["default_kb"] = None
        if config.get("default_kb") and not config["subject_defaults"].get(DEFAULT_SUBJECT):
            config["subject_defaults"][DEFAULT_SUBJECT] = config["default_kb"]
    ...
    def delete_kb_config(self, kb_name: str):
        ...
        subject_defaults = self._config.get("subject_defaults", {})
        for subject_name, default_kb in list(subject_defaults.items()):
            if default_kb == kb_name:
                subject_defaults[subject_name] = None
                removed = True

        if self._config.get("default_kb") == kb_name:
            self._config["default_kb"] = None
            removed = True
    ...
    def set_default_kb(self, kb_name: Optional[str]):
        ...
        if "subject_defaults" not in self._config:
            self._config["subject_defaults"] = _empty_subject_defaults()
        self._config["subject_defaults"][DEFAULT_SUBJECT] = kb_name
        self._save_config()
    ...
    def set_default_for_subject(self, subject: str, kb_name: Optional[str]):
        """Set the default knowledge base for a specific subject."""
        subject_name = normalize_subject(subject)
        if "subject_defaults" not in self._config:
            self._config["subject_defaults"] = _empty_subject_defaults()
        self._config["subject_defaults"][subject_name] = kb_name
        if subject_name == DEFAULT_SUBJECT:
            self._config["default_kb"] = kb_name
        self._save_config()
        logger.info(f"Set default KB for subject '{subject_name}': {kb_name}")

    def get_default_for_subject(self, subject: str) -> Optional[str]:
        """Get the default knowledge base for a specific subject."""
        subject_name = normalize_subject(subject)
        subject_defaults = self._config.get("subject_defaults", {})
        kb_name = subject_defaults.get(subject_name)
        if kb_name:
            return kb_name
        if subject_name == DEFAULT_SUBJECT:
