...
class TexChunker:
    ...
    def estimate_tokens(self, text: str) -> int:
        ...
        try:
            ...
            # If encoding fails, use rough estimate: 1 token 鈮?4 chars
            print(f"  鈿狅笍 Token estimation failed, using rough estimate: {e!s}")
            return len(text) // 4
    ...
    def _clean_text(self, text: str) -> str:
        ...
        for line in lines:
            if len(line) > 10000:  # Single line over 10k characters, may be problematic
                print(f"  鈿狅笍 Detected overly long line ({len(line)} characters), truncating")
