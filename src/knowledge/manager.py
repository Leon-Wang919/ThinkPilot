...
class KnowledgeBaseManager:
    ...
    def __init__(self, base_dir="./data/knowledge_bases"):
        ...
        try:
            self.default_subject = get_default_subject()
        except Exception:
    ...
    def _normalize_kb_entry(self, name: str, entry: dict | None) -> tuple[dict, bool]:
        payload = dict(entry or {})
        changed = False

        if payload.get("path") != name:
            payload["path"] = name
            changed = True

        if "description" not in payload:
            payload["description"] = f"Knowledge base: {name}"
            changed = True

        subject = payload.get("subject")
        try:
            normalized_subject = normalize_subject(subject or self.default_subject)
        except ValueError:
            normalized_subject = self.default_subject

        if payload.get("subject") != normalized_subject:
    ...
    def _load_config(self) -> dict:
        ...
        if self.config_file.exists():
            try:
                ...
                for name, entry in config.get("knowledge_bases", {}).items():
                    normalized_entry, changed = self._normalize_kb_entry(name, entry)
                    normalized[name] = normalized_entry
                    dirty = dirty or changed
                config["knowledge_bases"] = normalized
    ...
    def update_kb_status(
        ...
        if name not in self.config["knowledge_bases"]:
            ...
            self.config["knowledge_bases"][name] = {
                ...
                "subject": normalize_subject(subject or self.default_subject),
            }

        kb_config = self.config["knowledge_bases"][name]
        if subject:
            kb_config["subject"] = normalize_subject(subject)
        else:
            normalized_entry, _ = self._normalize_kb_entry(name, kb_config)
            kb_config.update(normalized_entry)
    ...
    def list_knowledge_bases(self, subject: str | None = None) -> list[str]:
        ...
        if subject:
            ...
            kb_list = [
                name
                for name in kb_list
                if config_kbs.get(name, {}).get("subject", self.default_subject) == subject_name
            ]

        # If no config file or config is empty, fallback to scanning directory (backward compatibility)
        if not kb_list and not config_kbs and self.base_dir.exists():
    ...
    def register_knowledge_base(
        ...
        if set_default:
            self.set_default_for_subject(subject_name, name)
    ...
    def set_default(self, name: str):
        """Compatibility wrapper that sets a KB as the default for its own subject."""
        self.set_default_for_subject(self.get_subject(name), name)
    ...
    def get_default(self) -> str | None:
        ...
        Priority:
        1. Default subject's subject-aware default
        2. Legacy global default in centralized config service
        3. First knowledge base in the list (auto-fallback)
        """
        subject_default = self.get_default_for_subject(self.default_subject)
        if subject_default:
    ...
    def get_subject(self, name: str) -> str:
        self.config = self._load_config()
        kb_config = self.config.get("knowledge_bases", {}).get(name, {})
        return normalize_subject(kb_config.get("subject", self.default_subject))

    def get_default_for_subject(self, subject: str | None = None) -> str | None:
        subject_name = normalize_subject(subject or self.default_subject)
        subject_kbs = self.list_knowledge_bases(subject_name)

        try:
            from src.services.config import get_kb_config_service

            kb_config_service = get_kb_config_service()
            configured = kb_config_service.get_default_for_subject(subject_name)
            if configured and configured in subject_kbs:
                return configured
        except Exception:
            pass

        try:
            configured = get_subject_config(subject_name).kb_name
        except Exception:
            configured = ""
        ...
        return subject_kbs[0] if subject_kbs else None

    def set_default_for_subject(self, subject: str, kb_name: str):
        subject_name = normalize_subject(subject)
        if kb_name not in self.list_knowledge_bases(subject_name):
            raise ValueError(
                f"Knowledge base '{kb_name}' is not available for subject '{subject_name}'"
            )
        ...
        try:
            ...
            kb_config_service = get_kb_config_service()
            kb_config_service.set_default_for_subject(subject_name, kb_name)
        except Exception as e:
            print(f"Warning: Failed to save subject default to centralized config: {e}")

    def set_subject(self, name: str, subject: str):
        if name not in self.list_knowledge_bases():
            raise ValueError(f"Knowledge base not found: {name}")

        subject_name = normalize_subject(subject)
        previous_subject = self.get_subject(name)
        self.config = self._load_config()
        kb_config = self.config.get("knowledge_bases", {}).get(name, {})
        kb_config["subject"] = subject_name
        self.config["knowledge_bases"][name] = kb_config
        self._save_config()

        metadata_file = self.base_dir / name / "metadata.json"
        if metadata_file.exists():
            try:
        ...
        if previous_subject != subject_name:
            previous_defaults = self.list_knowledge_bases(previous_subject)
            previous_replacement = previous_defaults[0] if previous_defaults else None
            try:
                from src.services.config import get_kb_config_service

                kb_config_service = get_kb_config_service()
                if kb_config_service.get_default_for_subject(previous_subject) == name:
                    kb_config_service.set_default_for_subject(
                        previous_subject, previous_replacement
                    )
            except Exception as e:
                print(f"Warning: Failed to repair previous subject default: {e}")

        if not self.get_default_for_subject(subject_name):
            self.set_default_for_subject(subject_name, name)

    def list_subject_knowledge_bases(self, subject: str | None = None) -> list[dict]:
        subject_name = normalize_subject(subject or self.default_subject)
        default_kb = self.get_default_for_subject(subject_name)
        results: list[dict] = []
        for kb_name in self.list_knowledge_bases(subject_name):
            info = self.get_info(kb_name)
            results.append(
                {
                    "name": kb_name,
                    "subject": subject_name,
                    "is_default": kb_name == default_kb,
    ...
    def get_info(self, name: str | None = None) -> dict:
        ...
        info = {
            "name": kb_name,
            "path": str(kb_dir),
            "subject": self.get_subject(kb_name),
            "metadata": {},
            "status": status,
            "progress": progress,
        }
        info["is_default"] = kb_name == self.get_default_for_subject(info["subject"])
    ...
    def delete_knowledge_base(self, name: str, confirm: bool = False) -> bool:
        ...
        subject_name = self.get_subject(name)
        self.config = self._load_config()
        kb_dir = self.get_knowledge_base_path(name)
        ...
        remaining_subject_kbs = self.list_knowledge_bases(subject_name)
        replacement = remaining_subject_kbs[0] if remaining_subject_kbs else None
        try:
            from src.services.config import get_kb_config_service

            kb_config_service = get_kb_config_service()
            if kb_config_service.get_default_for_subject(subject_name) == name:
                kb_config_service.set_default_for_subject(subject_name, replacement)
            kb_config_service.delete_kb_config(name)
        except Exception as e:
            print(f"Warning: Failed to repair centralized config after KB deletion: {e}")
