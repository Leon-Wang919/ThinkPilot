...
class InvestigateMemory:
    ...
    def update_evidence_state(
        ...
        *,
        ...
        covered_evidence: list[str] | None = None,
        unavailable_evidence: list[str] | None = None,
        knowns: list[str] | None = None,
        unknowns: list[str] | None = None,
        constraints: list[str] | None = None,
        satisfied_by_kb: list[str] | None = None,
        satisfied_by_web: list[str] | None = None,
        grounding_strength: str | None = None,
    ):
        def _merge_list(field_name: str, values: list[str] | None):
            if not values:
                return
            existing = self.metadata.get(field_name, [])
            merged = list(dict.fromkeys([*existing, *[item for item in values if item]]))
            self.metadata[field_name] = merged

        _merge_list("required_evidence", required_evidence)
        _merge_list("covered_evidence", covered_evidence)
        _merge_list("unavailable_evidence", unavailable_evidence)
        _merge_list("knowns", knowns)
        _merge_list("unknowns", unknowns)
        _merge_list("constraints", constraints)
        _merge_list("satisfied_by_kb", satisfied_by_kb)
        _merge_list("satisfied_by_web", satisfied_by_web)

        self.metadata["used_web_fallback"] = bool(self.metadata.get("satisfied_by_web"))
        if grounding_strength:
            self.metadata["grounding_strength"] = grounding_strength
        self.updated_at = datetime.now().isoformat()
