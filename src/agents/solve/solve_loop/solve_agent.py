...
class SolveAgent(BaseAgent):
    ...
    async def process(
        ...
        tool_plan = self._enforce_evidence_first_plan(
            question=question,
            current_step=current_step,
            tool_plan=tool_plan,
        )
    ...
    def _build_context(
        ...
        return {
            ...
            "current_role": current_step.role or self._infer_step_role(current_step),
    ...
    def _infer_step_role(self, current_step: SolveChainStep) -> str:
        explicit_role = (current_step.role or "").strip()
        if explicit_role:
            return explicit_role
        prefix, _, _ = (current_step.step_target or "").partition(":")
        return prefix.strip()

    def _step_has_direct_evidence(self, current_step: SolveChainStep) -> bool:
        if current_step.available_cite:
            return True
        for call in current_step.tool_calls:
            if call.status == "success" and (call.summary or call.raw_answer):
                return True
        return False

    def _build_forced_tool_plan(
        self, question: str, current_step: SolveChainStep
    ) -> list[dict[str, str]]:
        role = self._infer_step_role(current_step).lower()
        step_target = current_step.step_target.strip()

        if role in {"calculation", "verification"}:
            intent = (
                f"Independently compute or verify the result for this step with Python, then compare it "
                f"against the claimed conclusion.\nQuestion: {question}\nStep target: {step_target}"
            )
            return [{"type": "code_execution", "query": intent}]

        if role == "derivation":
            intent = (
                f"Retrieve or confirm the derivation basis and key formulas needed for this step.\n"
                f"Question: {question}\nStep target: {step_target}"
            )
            return [{"type": "rag_hybrid", "query": intent}]

        return []

    def _enforce_evidence_first_plan(
        self, question: str, current_step: SolveChainStep, tool_plan: list[dict[str, str]]
    ) -> list[dict[str, str]]:
        role = self._infer_step_role(current_step).lower()
        requires_tool_support = role in {"calculation", "derivation", "verification"}
        has_evidence = self._step_has_direct_evidence(current_step)

        if not requires_tool_support:
            return tool_plan

        if not tool_plan:
            if has_evidence:
                return tool_plan
            return self._build_forced_tool_plan(question, current_step)

        only_none = all(item.get("type") == "none" for item in tool_plan)
        if only_none and not has_evidence:
            return self._build_forced_tool_plan(question, current_step)

        if role in {"calculation", "verification"}:
            normalized_plan: list[dict[str, str]] = []
            for item in tool_plan:
                if item.get("type") == "none" and not has_evidence:
                    normalized_plan.extend(self._build_forced_tool_plan(question, current_step))
                else:
