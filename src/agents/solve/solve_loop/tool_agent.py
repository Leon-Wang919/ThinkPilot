...
"""
    ...
    async def run_code_intent(
        ...
        base_dir = Path(output_dir).resolve() if output_dir else Path().resolve()
        resolved_artifacts_dir = Path(artifacts_dir) if artifacts_dir else base_dir / "artifacts"
        resolved_artifacts_dir.mkdir(parents=True, exist_ok=True)
        return await self._execute_single_call(
            record=ToolCallRecord(tool_type="code_execution", query=intent, status="running"),
            kb_name="",
            output_dir=output_dir,
    ...
    async def _execute_single_call(
        ...
        if tool_type == "code_execution":
            ...
            if is_failed:
                ...
                error_prefix = "[Error] Code execution failed\n"
