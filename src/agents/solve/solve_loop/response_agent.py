...
class ResponseAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.enable_citations = config.get("solve", {}).get(
            "enable_citations",
            config.get("system", {}).get("enable_citations", True),
        )
    ...
    def _format_available_cite(
        ...
        for cite in step.available_cite:
            ...
            source_bucket = knowledge.metadata.get("source_bucket", "kb")
            lines.append(
                f"{cite} [{knowledge.tool_type}] [{source_bucket.upper()}]\n"
    ...
    def _extract_used_citations(self, content: str, step: SolveChainStep) -> list[str]:
        ...
        if not content:
            return []
        # Allow standard English brackets [cite], also tolerate some error formats
