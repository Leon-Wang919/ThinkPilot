...
class ManagerAgent(BaseAgent):
    ...
    async def process(
        ...
        steps = self._ensure_verification_step(question=question, steps=steps)
    ...
    def _ensure_verification_step(
        self, question: str, steps: list[SolveChainStep]
    ) -> list[SolveChainStep]:
        if not steps or not self._question_needs_verification(question, steps):
            return steps

        if any((step.role or "").strip().lower() == "verification" for step in steps):
            return steps

        cite_ids: list[str] = []
        for step in steps:
            cite_ids.extend(step.available_cite)
        ...
        return [
            *steps,
            SolveChainStep(
                ...
                role="Verification",
                step_target=(
                    "Verification: Check the final result against prior calculations, "
                    "cited evidence, and tool outputs; correct any inconsistency."
                ),
    ...
    def _question_needs_verification(self, question: str, steps: list[SolveChainStep]) -> bool:
        lowered_question = (question or "").lower()
        step_roles = {(step.role or "").strip().lower() for step in steps}
        ...
        if step_roles.intersection({"calculation", "derivation"}):
            return True
        if any(marker in lowered_question for marker in verification_markers):
            return True
        return any(char.isdigit() for char in lowered_question)
