...
{user_template.format(**knowledge)}"""
        ...
        grounding_block = "\n\n".join(
            part
            for part in [
                f"Selected knowledge base: {source_label}" if source_label else "",
                "Knowledge-base grounding context:",
                kb_context or "(No retrieved grounding context available.)",
                (
                    f"Grounding warning: {grounding_warning}\n"
                    "If the KB grounding is limited, be explicit and restrained. Do not pretend the KB covered more than it did."
                )
                if grounding_warning
                else (
                    "Requirement: Prioritize the KB grounding context. Use only light generic supplementation when needed,"
                    " and keep it consistent with the selected knowledge base."
                ),
            ]
            if part
        )
