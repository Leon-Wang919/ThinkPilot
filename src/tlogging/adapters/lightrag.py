...
Forwards LightRAG and RAG-Anything logs to ThinkPilot's unified logging system.
...
class LightRAGLogForwarder:
    """
    Handler that forwards LightRAG logger messages to ThinkPilot logger.
    """

    def __init__(self, thinkpilot_logger, add_prefix: bool = True):
        """
        Args:
            thinkpilot_logger: ThinkPilot Logger instance
            add_prefix: Whether to add [LightRAG] prefix to messages
        """
        import logging
        self.logging = logging
        self.thinkpilot_logger = thinkpilot_logger
    ...
    def emit(self, record):
        """
        Forward log record to ThinkPilot logger with proper level mapping.
        """
        try:
            message = record.getMessage()

            # Map LightRAG log levels to ThinkPilot logger methods
            level = record.levelno
            if level >= self.logging.ERROR:
                self.thinkpilot_logger.error(message)
            elif level >= self.logging.WARNING:
                self.thinkpilot_logger.warning(message)
            elif level >= self.logging.INFO:
                self.thinkpilot_logger.info(message)
            else:
                self.thinkpilot_logger.debug(message)
...
def LightRAGLogContext(logger_name: Optional[str] = None, scene: Optional[str] = None):
    ...
    # Get ThinkPilot logger
    thinkpilot_logger = get_logger(logger_name)
    ...
    forwarder = LightRAGLogForwarder(thinkpilot_logger, add_prefix=add_prefix)
