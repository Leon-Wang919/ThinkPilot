...
from src.services.setup import get_runtime_settings
...
app = FastAPI(
    ...
    # Disable trailing slash redirects to prevent protocol downgrade behind HTTPS proxies
    redirect_slashes=False,
)

project_root = Path(__file__).parent.parent.parent
runtimes = get_runtime_settings(project_root=project_root)

# Configure CORS
app.add_middleware(
    CORSMiddleware,
    allow_origins=runtimes.allowed_cors_origins,
...
# Physical Path: ThinkPilot/data/user/solve/solve_xxx/artifacts/image.png
...
# Include routers - current supported modules
...
app.include_router(feynman.router, prefix="/api/v1/feynman", tags=["feynman"])
...
@app.get("/")
async def root():
    return {"message": "Welcome to ThinkPilot API"}
