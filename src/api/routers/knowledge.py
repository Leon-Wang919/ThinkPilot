...
def get_knowledge_notebook_service() -> KnowledgeNotebookService:
    return KnowledgeNotebookService(get_kb_manager(), notebook_manager)
...
class SaveKnowledgeNotebookRecordRequest(BaseModel):
...
class UpdateKnowledgeBaseSubjectRequest(BaseModel):
    subject: str
...
async def health_check():
    ...
    return build_health_payload("knowledge")


@router.get("/status")
async def knowledge_status():
    """Detailed knowledge service status."""
    try:
        manager = get_kb_manager()
...
@router.get("/default")
async def get_default_kb(subject: str | None = Query(default=None)):
    """Get the default knowledge base for a subject."""
    try:
        manager = get_kb_manager()
        subject_name = normalize_subject(subject or get_default_subject())
        default_kb = manager.get_default_for_subject(subject_name)
        return {"subject": subject_name, "default_kb": default_kb}
    except Exception as e:
...
async def set_default_kb(kb_name: str, subject: str | None = Query(default=None)):
    """Set the default knowledge base for a subject."""
    try:
        manager = get_kb_manager()
        subject_name = normalize_subject(subject or get_default_subject())

        # Verify KB exists
        if kb_name not in manager.list_knowledge_bases(subject_name):
            raise HTTPException(status_code=404, detail=f"Knowledge base '{kb_name}' not found")

        manager.set_default_for_subject(subject_name, kb_name)
        return {"status": "success", "subject": subject_name, "default_kb": kb_name}
...
async def list_knowledge_bases(subject: str | None = Query(default=None)):
    """List available knowledge bases, optionally filtered by subject."""
    try:
        manager = get_kb_manager()
        kb_notebooks = get_knowledge_notebook_service()
        subject_name = normalize_subject(subject) if subject else None
        kb_names = manager.list_knowledge_bases(subject_name)
        ...
        for name in kb_names:
            try:
                info = kb_notebooks.attach_notebook_summary(manager.get_info(name))
                ...
                try:
                    ...
                    if kb_dir.exists():
                        ...
                        kb_subject = manager.get_subject(name)
                        result.append(
                            KnowledgeBaseInfo(
                                name=name,
                                subject=kb_subject,
                                is_default=name == manager.get_default_for_subject(kb_subject),
                                statistics={
                                    "raw_documents": 0,
                                    "images": 0,
                                    "content_lists": 0,
                                ...
                                notebook=kb_notebooks.ensure_notebook(name),
                            )
                        )
...
@router.put("/{kb_name}/subject")
async def update_knowledge_base_subject(kb_name: str, request: UpdateKnowledgeBaseSubjectRequest):
    """Move a knowledge base to another subject."""
    try:
        manager = get_kb_manager()
        subject_name = normalize_subject(request.subject)
        manager.set_subject(kb_name, subject_name)
        return {"status": "success", "kb_name": kb_name, "subject": subject_name}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
    except Exception as e:
        logger.error(f"Error updating KB subject for '{kb_name}': {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_knowledge_base_details(kb_name: str):
    ...
    try:
        manager = get_kb_manager()
        return get_knowledge_notebook_service().attach_notebook_summary(manager.get_info(kb_name))
    except ValueError:
        raise HTTPException(status_code=404, detail=f"Knowledge base '{kb_name}' not found")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@router.get("/{kb_name}/notebook")
async def get_knowledge_base_notebook(kb_name: str):
    """Get the dedicated notebook for a knowledge base."""
    try:
        notebook = get_knowledge_notebook_service().get_notebook(kb_name)
        return notebook
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
    except Exception as e:
...
@router.post("/{kb_name}/notebook/records")
async def save_knowledge_base_record(kb_name: str, request: SaveKnowledgeNotebookRecordRequest):
    """Save a conversation-derived record into the dedicated KB notebook."""
    try:
        result = get_knowledge_notebook_service().save_conversation_record(
            kb_name=kb_name,
...
@router.post("/{kb_name}/notebook/upload")
async def upload_knowledge_base_notebook_files(
    kb_name: str,
    files: list[UploadFile] = File(...),
):
    """Upload notebook files into the dedicated KB notebook."""
    try:
        ...
        if result["success_count"] + result.get("partial_count", 0) == 0:
            raise HTTPException(
                status_code=400,
                detail={
                    "message": "No notebook files were uploaded successfully",
...
async def delete_knowledge_base(kb_name: str):
    ...
    try:
        manager = get_kb_manager()
        kb_notebooks = get_knowledge_notebook_service()
        bound_notebook_id = kb_notebooks.get_bound_notebook_id(kb_name)
        ...
        if bound_notebook_id:
            notebook_manager.delete_notebook(bound_notebook_id, force=True)
...
async def create_knowledge_base(
    ...
    try:
        ...
        if name in manager.config.get("knowledge_bases", {}):
            ...
            manager.config["knowledge_bases"][name]["subject"] = subject_name
        ...
        notebook_summary = get_knowledge_notebook_service().ensure_notebook(name)
