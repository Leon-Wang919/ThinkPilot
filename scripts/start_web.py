...
def _load_setup_helpers():
    from src.services.setup import get_runtime_settings, init_user_directories, load_runtime_env

    return get_runtime_settings, init_user_directories, load_runtime_env


get_runtime_settings, init_user_directories, load_runtime_env = _load_setup_helpers()
...
def start_backend():
    ...
    settings = get_runtime_settings(Path(project_root))
...
def start_frontend():
    ...
    # Check frontend dependencies integrity.
    # node_modules may exist but be incomplete (e.g. interrupted install),
    # so we also verify the local next binary.
    node_modules_dir = os.path.join(web_dir, "node_modules")
    next_bin = os.path.join(node_modules_dir, ".bin", "next")
    needs_install = (not os.path.exists(node_modules_dir)) or (not os.path.exists(next_bin))
    ...
    if needs_install:
        ...
        try:
            ...
            install_env["PATH"] = f"{npm_dir}{os.pathsep}{install_env.get('PATH', '')}"
            ...
            if not os.path.exists(next_bin):
                raise RuntimeError(
                    "Frontend dependencies appear incomplete: node_modules/.bin/next is missing"
                )
        except Exception as e:
    ...
    settings = get_runtime_settings(Path(project_root))
    api_base_url = settings.resolved_api_base

    if settings.next_public_api_base_external:
        print_flush(f"📌 Using external API URL from env: {api_base_url}")
    elif settings.next_public_api_base:
...
if __name__ == "__main__":
    ...
    print_flush("=" * 50)
    print_flush("ThinkPilot Web Platform Launcher")
    print_flush("=" * 50)
    ...
    try:
        ...
        backend_port = get_runtime_settings(
        ...
        backend_port, frontend_port = (
            runtime_settings.backend_port,
            runtime_settings.frontend_port,
        )
